...
impl<R: Read> Deserializer<R> {
    ...
    //
    // PATCHED (memviz): always clone, never remove. Upstream's ref-count
    // logic gets out-of-sync on PyTorch memory snapshots (huge shared frames
    // lists), resulting in None values inside otherwise-valid frames.
    // Keeping memo entries alive until parse completes is slightly heavier
    // on peak memory but preserves correctness.
    fn resolve_recursive<T, U, F>(&mut self, id: MemoId, u: U, f: F) -> Result<T>
    where
        F: FnOnce(&mut Self, U, Value) -> Result<T>,
    {
        let value = match self.memo.get(&id) {
            Some((v, _)) => v.clone(),
            None => {
